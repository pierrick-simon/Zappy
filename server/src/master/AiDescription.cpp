...
namespace Zappy {
    AiDescription::AiDescription(
        sf::Font &font, const std::unordered_map<std::string, std::string> &ais)
    {
    ...
    void AiDescription::draw(sf::RenderWindow &win)
    {
        ...
        for (std::size_t i = 0; i < _descriptions.size(); i++) {
            _descriptions[i].setHeight(Init::TOOLBAR_SIZE_Y + Init::BOX_GAP +
                float(i) * (Init::BOX_GAP + Init::DESC_BOX_Y) - _scroll);
            _descriptions[i].draw(win);
        }
    }
    ...
    void AiDescription::event(sf::Event &event, sf::Vector2f mousePos)
    {
        if (_rec.getGlobalBounds().contains(mousePos) &&
            ...
            auto max = (Init::BOX_GAP + Init::DESC_BOX_Y) *
                    float(_descriptions.size()) -
                Init::AI_BOX_Y + Init::BOX_GAP;
            if (max <= 0)
                _scroll = 0;
            else
